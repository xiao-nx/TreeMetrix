...
#保存图像
def saveDstImage(image,label,images_output,labels_output,labels_decode_output,count):
  ...
  cv2.imwrite(labels_decode_output + 'label_decode_{}.png'.format(count),label_decode)
...
def create_augmented_images(data_path_dict):
    ...
    labels_decode_output = data_path_dict["label_decode_output"]

    if not os.path.exists(images_output):
        os.makedirs(images_output)
    if not os.path.exists(labels_output):
        os.makedirs(labels_output)
    if not os.path.exists(labels_decode_output):
        os.makedirs(labels_decode_output)
    ...
    for idx,fname in enumerate(images_list):
        ...
        saveDstImage(image_no_white_boder, label_no_white_boder,images_output,labels_output,labels_decode_output,count)
        ...
        for i in range(_NUM_DEG):
            ...
            saveDstImage(rotated_image, rotated_label,images_output,labels_output,labels_decode_output,count)           
            count += 1
            if np.random.random() < 0.01 :
              equ_image,equ_label = imtrans.randomHueSaturationValue(rotated_image, rotated_label)
              saveDstImage(equ_image,equ_label,images_output,labels_output,labels_decode_output,count)
              count += 1
            ...
            for j in range(_NUM_PERPECTIVE_WARP):
              ...
              saveDstImage(perspective_image, perspective_label,images_output,labels_output,labels_decode_output,count)
              count += 1
              if np.random.random() < 0.001 :
                G_noise_img,G_noise_label = imtrans.addGaussianNoise(perspective_image, perspective_label)
                saveDstImage(G_noise_img,G_noise_label,images_output,labels_output,labels_decode_output,count)
                count += 1
              
              for k in range(_NUM_MLS_WARP):
                  #mlswarp
                  mlswarp_image, mlswarp_label = imtrans.get_mlswarp_image(perspective_image, perspective_label)
                  saveDstImage(mlswarp_image, mlswarp_label,images_output,labels_output,labels_decode_output,count)
                  count += 1
                  if np.random.random() < 0.001 :
                    SP_noise_img,SP_noise_label = imtrans.addSaltAndPepperNoise(rotated_image, rotated_label)
                    saveDstImage(SP_noise_img,SP_noise_label,images_output,labels_output,labels_decode_output,count)
                    count += 1
...
if __name__ == "__main__":
  data_path_dict = {
    "image_input" : "../data_raw/images/training/",
    "label_input" : "../data_raw/annotations/training/",
    "image_output" : "../data_augmented/images/training/",
    "label_output" : "../data_augmented/annotations/training/",
    "label_decode_output":"../data_augmented/annotations/training_decode/"
    }
