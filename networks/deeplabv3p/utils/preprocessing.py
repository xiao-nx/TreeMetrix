  ...
  for i in range(num_images):
    ...
    for j_, j in enumerate(mask[i, :, :, 0]):
      for k_, k in enumerate(j):
        if k < num_classes:
          pixels[k_, j_] = label_colours[k % num_classes]
...
def mean_image_subtraction(image, means=(_R_MEAN, _G_MEAN, _B_MEAN)):
  ...
  if image.get_shape().ndims != 3:
    raise ValueError('Input must be of size [height, width, C>0]')
  num_channels = image.get_shape().as_list()[-1] #[N, H, W, C]
  if len(means) != num_channels:
    raise ValueError('len(means) must match the number of channels')
...
def random_flip_left_right_image_and_label(image, label):
  ...
  image = tf.cond(mirror_cond, lambda: tf.reverse(image, [2]), lambda: image) #2是width，水平反转
  label = tf.cond(mirror_cond, lambda: tf.reverse(label, [2]), lambda: label)
