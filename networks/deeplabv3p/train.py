...
#resnet_v1是ResNet，resnet_v2是ResNeXt
parser.add_argument('--base_architecture', type=str, default='mobilenet_v1',
                    choices=['mobilenet_v1'],
                    help='The architecture of base Mobilenet building block.')
...
#input_fn第一个map
#运行过程中，dataset API会将raw_record=tf_example.SerializeToString()
...
def preprocess_image(image, label, is_training):
  ...
  if is_training:
    ...
    #ignore label不参与负反馈，也可以填0背景参与负反馈
  ...
  image = preprocessing.mean_image_subtraction(image) #减去均值
...
def input_fn(is_training, data_dir, batch_size, num_epochs=1):
  ...
  dataset = dataset.flat_map(tf.data.TFRecordDataset) #还有tf.data.ImageReadDataset读取单张图像
  ...
  #对dataset每一个数据做如下映射
  #从tf_example中解析出原始的image/label
  dataset = dataset.map(parse_record)
  #抽取相同大小的image/label组成batch训练，输入是parse_record的输出
...
def main(unused_argv):
  ...
  run_config = tf.estimator.RunConfig().replace(save_checkpoints_secs=_SAVE_CKPT_SECS)
  model = tf.estimator.Estimator(
      model_fn=deeplab_model.deeplabv3_plus_model_fn, #训练函数，输入input_fn image/label, params参数
      model_dir=FLAGS.model_dir, #模型保存路径
      config=run_config,
      params={
          'output_stride': FLAGS.output_stride,
          'batch_size': FLAGS.batch_size,
          'base_architecture': FLAGS.base_architecture,
          'pre_trained_model': FLAGS.pre_trained_model, #预训练模型，resnet50/101
  ...
  for _ in range(FLAGS.train_epochs // FLAGS.epochs_per_eval):
    ...
    model.train(
        input_fn=lambda: input_fn(True, FLAGS.data_dir, FLAGS.batch_size, FLAGS.epochs_per_eval), #训练时，给model_fn提供数据的函数
        hooks=train_hooks,
        # steps=1  # For debug
    )
